...
<v-dialog v-model="isConfirmClear" persistent max-width="400">
  <v-card>
    ...
    <v-card-actions>
      <v-spacer></v-spacer>
      <v-btn  class=" bg-mygreen" color="white rounded-lg" text @click="confirmClearCart">Yes</v-btn>
...
" class="  font-weight-medium px-1  text-left">
        <v-checkbox color="mygreen" v-model="selectAll" hide-details ></v-checkbox>
     </th>
 ...
 <tbody>
     ...
     <td  class="text-grey-lighten-1 pl-1 ">   
            <v-checkbox color="mygreen" v-model="item.selected" hide-details></v-checkbox>
     </td>
...
<v-chip size="x-small" color="mygreen" style="font-weight: 500;" class="mt-1" rounded="lg">IN STOCK</v-chip>
        </div>
      </div>
      </div>
...
<v-dialog v-model="isModalVisible" persistent max-width="400">
    <v-card>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn class="bg-mygreen "  color="white rounded-lg" text @click="confirmRemoval">Yes</v-btn>
...
<div class="d-flex mt-4 align-center">
  <v-icon class="mr-2" icon="mdi mdi-information" color="mygreen" size="18"></v-icon>
  <p class="textClass text-grey">Delivery fees not Included</p>
</div>

</div>
<v-btn to="/order/checkout" flat block size="x-large" class="mt-8" rounded="xl" color="mygreen"><span style="font-size: 14px;
font-style: normal;
